...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// The sidebar foot stacks `footerActions` (our icon) above `settingsArea`
		// (settings) because footArea is a flex column and both children are
		// width:100%. Make the foot a horizontal row so the phone icon sits
		// BESIDE settings. Substring matchers track the CSS-module naming
		// convention without hardcoding the hash prefix.
		(function injectRailCss() {
			try {
				var id = 'dsh-mobile-access-foot-css';
				if (document.getElementById(id)) return;
				var tag = document.createElement('style');
				tag.id = id;
				tag.textContent =
					'[class*="footArea"]{flex-direction:row-reverse !important;align-items:center;gap:20px}' +
					'[class*="footArea"]>[class*="footerActions"],[class*="footArea"]>[class*="settingsArea"]{width:auto !important;flex:0 0 auto !important}';
