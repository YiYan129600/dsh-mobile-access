...
function detectTailnetSuffix() {
  ...
  return new Promise((resolve) => {
    execFile('ipconfig', ['/all'], { timeout: 4000, windowsHide: true, maxBuffer: 1024 * 1024 }, (err, stdout) => {
      if (err || !stdout) return resolve(null);
      try {
        tailnetSuffixCache = parseTailscaleSuffix(String(stdout));
      } catch {}
      resolve(tailnetSuffixCache);
    });
  });
}
