...
export function welcomePageHtml(pairState, device, pagePath) {
...
function statusJson(state, pairState, hubCount) {
...
export function apply(ctx, config = {}) {
  ...
  // Notify connected phones of agent/approval/todo activity (lightweight kinds
  // only, no message bodies — the channel clears the DNS fence, not auth).
  const offSessionEvents = ctx.on('session/event', (session, event) => {
    const classified = classifySessionEvent(event);
    if (!classified) return;
    hub.publish({ kind: classified.kind, sessionId: session?.id ?? null, ts: Date.now() });
  });
  ...
  const route = {
    ...
    handler: async (req, res) => {
      ...
      if (url.startsWith(`${api}/pair/mint`)) {
        ...
        hub.publish({ kind: 'pair.changed', phase: 'waiting', ts: Date.now() });
      ...
      if (url.startsWith(`${api}/pair/accept`)) {
        ...
        hub.publish({ kind: 'pair.changed', phase: 'connected', deviceId: result.id, ts: Date.now() });
      ...
      if (url.startsWith(`${api}/pair/revoke`)) {
        ...
        if (revoked) hub.publish({ kind: 'pair.changed', phase: 'revoked', deviceId: id, ts: Date.now() });
      ...
      if (url.startsWith(`${api}/pair/stop`)) {
        ...
        pair.stop();
        hub.publish({ kind: 'pair.changed', phase: 'stopped', ts: Date.now() });
      ...
      if (url.startsWith(`${api}/events`)) {
        ...
        req.on('close', () => {
          clearInterval(heartbeat);
          hub.remove(client);
          hub.publish({ kind: 'device.offline', deviceId: device.id, ts: Date.now() });
        });
        hub.publish({ kind: 'device.online', deviceId: device.id, ts: Date.now() });
